...
func (q *query) windowVars() *query {
	return q.vars(
		...
		varWindowManualHeight,
		varWindowManualWidth,
		varWindowMarkedFlag,
		varWindowModalPane,
		varWindowName,
		varWindowOffsetX,
...
func (r queryResult) toWindow(t *Tmux) *Window {
	window := &Window{
		...
		ManualHeight:       atoi(r.get(varWindowManualHeight)),
		ManualWidth:        atoi(r.get(varWindowManualWidth)),
		MarkedFlag:         isOne(r.get(varWindowMarkedFlag)),
		ModalPane:          r.get(varWindowModalPane),
